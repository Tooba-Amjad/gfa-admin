...
class FirebaseApi {
    ...
    FirebaseFirestore.instance.runTransaction((Transaction tx) async {
      ...
      if (docSnapshot.exists) {
        ...
        if (ind >= 0) {
          ...
          tx.update(newrefdata, listkeyname == null ? {Dbkeys.list: newlist} : {listkeyname: newlist});
        } else {
          if (isshowloader == true) {
      ...
      onErrorFn('${getTranslatedForCurrentUser(context!, 'xxxfailedntryagainxxx')}\n $error');
  ...
  static Future<void> runUPDATEmapobjectinListField({
    ...
    FirebaseFirestore.instance.runTransaction((Transaction tx) async {
      ...
      if (docSnapshot.exists) {
        ...
        List mykeylist = replaceableMapObjectWithOnlyFieldsRequired.keys.toList();
        List myvaluelist = replaceableMapObjectWithOnlyFieldsRequired.values.toList();
        int ind = newlist!.lastIndexWhere((dc) => dc[compareKey] == compareVal);
  ...
  static Future<void> runTransactionRecordActivity({
    ...
    DocumentReference docRef = FirebaseFirestore.instance.collection(DbPaths.adminapp).doc(DbPaths.collectionhistory);
    if (isshowloader == true) {
      ShowLoading().open(context: context!, key: keyloader);
    }
    if (isOnlyAlertNotSave == true) {
      ...
      FirebaseFirestore.instance.runTransaction((Transaction tx) async {
        ...
        if (docSnapshot.exists) {
          ...
          onErrorFn("Document Does not exist. Please contact the developer. DOCREF: ${docRef.toString()}");
        }
      }).then((result) {
        if (isshowloader == true) {
          ShowLoading().close(context: context, key: keyloader!);
...
//-----UPDATED
  ...
  static Future<DocumentSnapshot> getFirestoreDOCUMENTData(String? dataType, DocumentReference refdata) async {
    return refdata.get();
  }
  ...
  Future<void> runUPDATEtransaction({
    ...
    FirebaseFirestore.instance.runTransaction((Transaction tx) async {
      ...
      if (docSnapshot.exists) {
        ...
        if (isincremental == false || isincremental == null) {
          if (newstringinlist == null) {
            ...
            if (ind >= 0) {
              ...
              tx.update(newrefdata, listreplaceablekey == null ? {Dbkeys.list: newlist} : {listreplaceablekey: newlist});
            } else {
              if (isshowloader == null || isshowloader == true) {
                ShowLoading().close(context: context, key: keyloader!);
              }
              ...
              ShowSnackbar().open(
                  context: context, scaffoldKey: scaffoldkey as GlobalKey<ScaffoldState>, time: 4, status: 1, label: 'Field doesn\'t exists ! Please reload this page.');
            }
          } else {
            if (isreplace == false || isreplace == null) {
              ...
              int ind = newlist!.indexWhere((element) => element.toString() == compareKey.toString());
              newlist.removeAt(ind);
          ...
          int ind = newlist!.indexWhere((dc) => dc[compareKey].contains(compareVal));
        ...
        ShowSnackbar().open(
            context: context, scaffoldKey: scaffoldkey as GlobalKey<ScaffoldState>, time: 4, status: 1, label: 'Document doesn\'t exists ! Please reload this page.');
      }
    }).then((result) {
      ...
      if (isshowmsg == true || isshowmsg == null) {
        ShowSnackbar()
            .open(context: context, scaffoldKey: scaffoldkey as GlobalKey<ScaffoldState>, time: 2, status: 2, label: 'Success ! Operation successfully performed.');
      }
      ...
      ShowCustomAlertDialog().open(
          ...
          description: '${getTranslatedForCurrentUser(context, 'xxxfailedntryagainxxx')}\n $error');
    });
  }
      ...
      if (docSnapshot.exists) {
        ...
        if (isincremental == false || isincremental == null) {
          if (insertMapInListfield != null) {
            Map<String, dynamic> updateobject = docSnapshot.data() as Map<String, dynamic>;
  ...
  Future<void> runUPDATEtransactionWithQuantityCheck({
    ...
    FirebaseFirestore.instance.runTransaction((Transaction tx) async {
      DocumentSnapshot docSnapshot = await tx.get(refdata ?? FirebaseFirestore.instance.collection(DbPaths.collectionhistory).doc(DbPaths.collectionhistory));
      ...
      if (docSnapshot.exists) {
        ...
        tx.update(refdata ?? FirebaseFirestore.instance.collection(DbPaths.collectionhistory).doc(DbPaths.collectionhistory),
  ...
  Future<void> runDELETEtransaction({
    ...
    FirebaseFirestore.instance.runTransaction((Transaction tx) async {
      ...
      if (docSnapshot.exists) {
        ...
        int ind = iswholeelementcompareable == false || iswholeelementcompareable == null
            ? newlist!.indexWhere((dc) => dc[compareKey] == compareVal)
            : newlist!.indexWhere((dc) => dc.toString() == compareKey.toString());
        print(ind);
        if (ind >= 0) {
          // newlist[ind][];
          newlist.removeAt(ind);

          tx.update(refdata, listalternative == null ? {Dbkeys.list: newlist} : {listalternative: newlist});
        } else {
          if (isshowloader == null || isshowloader == true) {
            ShowLoading().close(context: context, key: keyloader!);
          }
